-- [[ PHUC NGUYEN X HUB - FINAL V10.7 - REWORKED HITBOX & DAMAGE ]] --
...
-- [CẤU HÌNH]
getgenv().Config = {
    Hitbox = false, Size = 50, Trans = 5, Aura = false,
                ...
                if getgenv().TargetKill then
                    ...
                    if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        elseif TarHum.Health > 0 then
                            ...
                            if Dist > (getgenv().Config.Size / 2) + 5 then 
    ...
    if getgenv().Config.Hitbox then
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and not getgenv().Whitelist[v.Name] and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                HRP.CanQuery = true -- Cho phép nhận diện tia quét
                HRP.CanTouch = true -- Cho phép va chạm gây dame
    ...
    while task.wait() do 
        pcall(function()
            if getgenv().Config.Aura then
                ...
                for _, v in pairs(Players:GetPlayers()) do
                    if v ~= LocalPlayer and not getgenv().Whitelist[v.Name] and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                        local TarRoot = v.Character.HumanoidRootPart
                        local TarHum = v.Character:FindFirstChild("Humanoid")
                        
                        if TarHum and TarHum.Health > 0 then
                            -- Tầm quét tự động co giãn theo Size Hitbox
                            local Radius = (getgenv().Config.Size / 2) + 5
...
Notify("PHUC NGUYEN HUB V10.7 - HITBOX REWORKED!")
