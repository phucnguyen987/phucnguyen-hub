...
-- [[ BOX MUA NHANH BĂNG GẠC - QUICK BUY ]] --
local BuyBox = Instance.new("Frame", Container); BuyBox.Size = UDim2.new(1, 0, 0, 110); BuyBox.BackgroundColor3 = Color3.fromRGB(30, 30, 35); Instance.new("UICorner", BuyBox); MakeRainbowStroke(BuyBox, 1.5)

local BuyTitle = Instance.new("TextLabel", BuyBox); BuyTitle.Size = UDim2.new(1, 0, 0, 25); BuyTitle.Text = "🛒 MUA NHANH BĂNG GẠC"; BuyTitle.TextColor3 = Color3.new(1, 1, 1); BuyTitle.Font = "FredokaOne"; BuyTitle.TextSize = 14; BuyTitle.BackgroundTransparency = 1

-- Hàm thực thi mua
local function ExecuteBuy(Amount)
    ...
    task.spawn(function()
        ...
        Notify("Đã mua xong " .. count .. " Băng Gạc!")
    end)
end

-- Hàng nút ngang x10, x50, x99
local Row = Instance.new("Frame", BuyBox); Row.Size = UDim2.new(1, -10, 0, 30); Row.Position = UDim2.new(0, 5, 0, 30); Row.BackgroundTransparency = 1
local RowList = Instance.new("UIListLayout", Row); RowList.FillDirection = "Horizontal"; RowList.Padding = UDim.new(0, 5); RowList.HorizontalAlignment = "Center"

local function CreateQuickBtn(Text, Val)
    local B = Instance.new("TextButton", Row); B.Size = UDim2.new(0, 60, 1, 0); B.BackgroundColor3 = Color3.fromRGB(50, 50, 60); B.Text = Text; B.TextColor3 = Color3.new(1, 1, 1); B.Font = "FredokaOne"; B.TextSize = 14; Instance.new("UICorner", B)
    B.MouseButton1Click:Connect(function() ExecuteBuy(Val) end)
end
CreateQuickBtn("x10", 10); CreateQuickBtn("x50", 50); CreateQuickBtn("x99", 99)

-- Box nhập số và nút Buy
local InputBox = Instance.new("TextBox", BuyBox); InputBox.Size = UDim2.new(0.6, -10, 0, 30); InputBox.Position = UDim2.new(0, 5, 0, 70); InputBox.PlaceholderText = "Nhập số (1-99)..."; InputBox.Text = ""; InputBox.BackgroundColor3 = Color3.fromRGB(20, 20, 20); InputBox.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", InputBox)
local BuyBtn = Instance.new("TextButton", BuyBox); BuyBtn.Size = UDim2.new(0.4, -5, 0, 30); BuyBtn.Position = UDim2.new(0.6, 0, 0, 70); BuyBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 0); BuyBtn.Text = "MUA"; BuyBtn.TextColor3 = Color3.new(1, 1, 1); BuyBtn.Font = "FredokaOne"; Instance.new("UICorner", BuyBtn)
BuyBtn.MouseButton1Click:Connect(function() ExecuteBuy(InputBox.Text) end)
...
-- [ SPIN BOT MƯỢT - GIỮ NGUYÊN CFRAME ] --
local SpinAngle = 0
...
RunService.RenderStepped:Connect(function()
    if getgenv().Config.FakeVip and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head") then
        local Head = LocalPlayer.Character.Head
        local Tag = Head:FindFirstChild("PhucVipTag") or Instance.new("BillboardGui", Head)
        Tag.Name = "PhucVipTag"; Tag.Size = UDim2.new(0, 120, 0, 50); Tag.AlwaysOnTop = true; Tag.ExtentsOffset = Vector3.new(0, 3, 0)
        local Label = Tag:FindFirstChild("Label") or Instance.new("TextLabel", Tag)
        Label.Name = "Label"; Label.Size = UDim2.new(1, 0, 1, 0); Label.BackgroundTransparency = 1; Label.Text = getgenv().Config.VipTag; Label.TextColor3 = getgenv().Config.VipColor; Label.Font = "FredokaOne"; Label.TextSize = 20; Label.TextStrokeTransparency = 0
    elseif not getgenv().Config.FakeVip and LocalPlayer.Character and LocalPlayer.Character.Head:FindFirstChild("PhucVipTag") then
        LocalPlayer.Character.Head.PhucVipTag:Destroy()
    end
end)

function AddSpecToggle(Text, Flag)
    local Btn = Instance.new("TextButton", SpecContainer); Btn.Size = UDim2.new(1, 0, 0, 35); Btn.BackgroundColor3 = Color3.fromRGB(35, 35, 35); Btn.Text = "  " .. Text .. " [OFF]"; Btn.TextColor3 = Color3.fromRGB(150, 150, 150); Btn.TextXAlignment = "Left"; Btn.Font = "SourceSansBold"; Instance.new("UICorner", Btn)
...
function AddSpecSlider(Text, Flag, Min, Max)
    local F = Instance.new("Frame", SpecContainer); F.Size = UDim2.new(1, 0, 0, 45); F.BackgroundTransparency = 1
    ...
    local Fill = Instance.new("Frame", B); Fill.Size = UDim2.new(0,0,1,0); Fill.BackgroundColor3 = Color3.new(0,1,1)
...
AddSpecToggle("Spin Bot 360 (No Stutter)", "SpinBot")
AddSpecSlider("Tốc Độ Xoay", "SpinSpeed", 0, 100)
AddSpecToggle("Fake VIP Tag", "FakeVip")
local EditTag = Instance.new("TextBox", SpecContainer); EditTag.Size = UDim2.new(1,0,0,35); EditTag.PlaceholderText = "Nhập chữ VIP..."; EditTag.Text = ""; EditTag.BackgroundColor3 = Color3.fromRGB(45,45,45); EditTag.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", EditTag)
EditTag.FocusLost:Connect(function() getgenv().Config.VipTag = EditTag.Text end)
local ColorBtn = Instance.new("TextButton", SpecContainer); ColorBtn.Size = UDim2.new(1,0,0,35); ColorBtn.BackgroundColor3 = Color3.new(1,1,0); ColorBtn.Text = "Đổi Màu Tag"; ColorBtn.TextColor3 = Color3.new(0,0,0); Instance.new("UICorner", ColorBtn)
ColorBtn.MouseButton1Click:Connect(function() getgenv().Config.VipColor = Color3.fromHSV(tick()%5/5, 1, 1); ColorBtn.BackgroundColor3 = getgenv().Config.VipColor end)
local BuyRemoteBtn = Instance.new("TextButton", SpecContainer); BuyRemoteBtn.Size = UDim2.new(1, 0, 0, 35); BuyRemoteBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 0); BuyRemoteBtn.Text = "🛒 MỞ SHOP TỪ XA"; BuyRemoteBtn.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", BuyRemoteBtn)
BuyRemoteBtn.MouseButton1Click:Connect(function() PlayClickSound(); local f = false; for _,v in pairs(game:GetDescendants()) do if v:IsA("RemoteEvent") and (v.Name:lower():find("shop") or v.Name:lower():find("buy")) then v:FireServer(); f = true end end; Notify(f and "Đã kích hoạt Shop!" or "Không tìm thấy Shop.") end)

-- [ AUTO ATTACK - FIX CHAT ] --
spawn(function()
        ...
        pcall(function()
            if getgenv().Config.Aura then
                if not UserInputService:GetFocusedTextBox() then
                    ...
                    if getgenv().Config.HitSound then
                        local s = Instance.new("Sound", workspace); s.SoundId = "rbxassetid://169444938"; s.Volume = 1; s:Play(); game:GetService("Debris"):AddItem(s, 1)
                    end
                end
            end
        end)
...
HideBtn.MouseButton1Click:Connect(function() PlayClickSound(); GUIVisible = not GUIVisible; Toggle.Visible = GUIVisible; Main.Visible = false; SubMenu.Visible = false; SubMenu2.Visible = false; SpecialMenu.Visible = false; HideIcon.Text = GUIVisible and "👁️" or "🚫" end)

spawn(function()
    local Angle = 0; local AuraBackup = false
    while task.wait() do
        pcall(function()
                ...
                if getgenv().TargetKill then
                    ...
                    if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if TarHum.Health > 0 and TarHum.Health < 3 then
                            KillStatus.Text = ">>> ĐANG ĐỨNG ĐẬP KẾT LIỄU! <<<"; StandUp(); MyRoot.CFrame = TarRoot.CFrame; MyRoot.Velocity = Vector3.zero
                        ...
                        elseif TarHum.Health > 0 then
                            ...
                            KillStatus.Text = ">>> ĐANG SĂN: " .. Target.Name .. " <<<"; MyRoot.AssemblyLinearVelocity = Vector3.zero; MyHum.PlatformStand = true
                            if Dist > 10 then 
                            ...
                            else Angle = Angle + 0.3; local CirclePos = TarRoot.Position + Vector3.new(math.cos(Angle) * 6, 2, math.sin(Angle) * 6); MyRoot.CFrame = CFrame.new(CirclePos, TarRoot.Position) end
                        else ShowKillNotify(getgenv().TargetKill); getgenv().TargetKill = nil; KillStatus.Text = ""; MyHum.PlatformStand = false; if AuraBackup then getgenv().Config.Aura = true; AuraBackup = false end end
                    else getgenv().TargetKill = nil; KillStatus.Text = "" end
                else KillStatus.Text = ""; if MyHum.PlatformStand then MyHum.PlatformStand = false end end
            end
        end)
    end
end)
...
local PlayerBtn = Instance.new("TextButton", Container); PlayerBtn.Size = UDim2.new(1,0,0,35); PlayerBtn.BackgroundColor3 = Color3.fromRGB(40,40,40); PlayerBtn.Text = ">>> MỞ PLAYER TRACKER (KILL/ESP) <<<"; PlayerBtn.TextColor3 = Color3.new(0,1,1); PlayerBtn.Font = "SourceSansBold"; Instance.new("UICorner", PlayerBtn)
PlayerBtn.MouseButton1Click:Connect(function() PlayClickSound(); if Main.Visible then SubMenu.Visible = not SubMenu.Visible; SubMenu2.Visible = false; SpecialMenu.Visible = false; if SubMenu.Visible then RefreshPlayers() end end end)
AddToggle("Auto Attack (Chat OK)", "Aura")
    ...
    if getgenv().Config.Hitbox then
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and not getgenv().Whitelist[v.Name] and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                HRP.Transparency = getgenv().Config.Trans/10; HRP.CanCollide = false; HRP.CanQuery = true; HRP.CanTouch = true; HRP.BrickColor = BrickColor.new("Really red")
            end
        end
    end
end)
RunService.Stepped:Connect(function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if getgenv().Config.Speed and Hum.MoveDirection.Magnitude > 0 then
            Hum.WalkSpeed = 16; local ExtraSpeed = (getgenv().Config.WalkSpeed - 16) * 0.02; HRP.CFrame = HRP.CFrame + (Hum.MoveDirection * ExtraSpeed)
...
Toggle.MouseButton1Click:Connect(function() PlayClickSound(); Main.Visible = not Main.Visible; SubMenu.Visible = false; SubMenu2.Visible = false; SpecialMenu.Visible = false end)
