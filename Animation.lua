...
getgenv().autoReloadEnabled = true -- Đã chỉnh mặc định thành true để tự nạp lại khi reset
...
local function onCharacterAdded(character)
    ...
    -- Không stopEmote ở đây để tránh bị mất trạng thái khi respawn
    
    local humanoid = character:WaitForChild("Humanoid")
    local animator = humanoid:WaitForChild("Animator")

    -- FIX: Tự động nạp lại animation nếu có lưu cache
    if getgenv().autoReloadEnabled and getgenv().lastPlayedAnimation then
        task.wait(0.5) -- Đợi nhân vật ổn định
        applyAnimation(getgenv().lastPlayedAnimation)
        getgenv().Notify({
            Title = '7yd7 | Auto Reload',
            Content = '🔄 Restored animation!',
            Duration = 3
        })
    end
    ...
    -- Đã xóa logic reset các biến enabled về false khi nhân vật chết
