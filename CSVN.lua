...
-- [ CẬP NHẬT SPIN BOT MƯỢT BẰNG VẬT LÝ ] --
RunService.Stepped:Connect(function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        local HRP = LocalPlayer.Character.HumanoidRootPart
        
        -- Fix Spin Bot bằng AngularVelocity để không bị khựng khi di chuyển
        local Spin = HRP:FindFirstChild("PhucSpinBody")
        if getgenv().Config.SpinBot then
            if not Spin then
                Spin = Instance.new("AngularVelocity", HRP)
                Spin.Name = "PhucSpinBody"
                Spin.MaxTorque = math.huge
                Spin.Attachment0 = HRP:FindFirstChild("RootAttachment") or Instance.new("Attachment", HRP)
            end
            Spin.AngularVelocity = Vector3.new(0, getgenv().Config.SpinSpeed, 0)
        else
...
AddSpecToggle("Spin Bot 360 (Vật Lý Mượt)", "SpinBot")
    ...
    while task.wait(0.1) do
        pcall(function()
            if getgenv().Config.Aura then
                -- Kiểm tra nếu người chơi đang gõ chat thì không Click
...
AddToggle("Auto Attack (Bật Chat OK)", "Aura")
