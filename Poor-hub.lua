...
getgenv().Config = {
    ...
    HitSound = true,
    SkyWalk = false,
    FlySpeed = 20 -- (Biến này giữ lại để tránh lỗi, nhưng logic sẽ dùng WalkSpeed)
}
getgenv().Whitelist = {}
...
-- [[ ĐÃ SỬA: AUTO KILL & STOMP (BYPASS FLY + CIRCLE + LINKED SPEED) ]] --
spawn(function()
    local Angle = 0
    while task.wait() do
        -- Nếu có mục tiêu
        if getgenv().TargetKill then
            ...
            if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                elseif TarHum.Health > 0 then
                    -- BẬT CHẾ ĐỘ BAY (BYPASS PHYSICS + ANTI-SINK)
                    MyRoot.Velocity = Vector3.new(0, 0, 0) -- Xóa quán tính cũ
                    MyRoot.AssemblyLinearVelocity = Vector3.new(0, 0, 0) -- [FIX LÚN] Khóa vật lý mới, chống rơi
                    ...
                    if Dist > 10 then 
                        ...
                        -- [TỐC ĐỘ ĐỒNG BỘ] Lấy từ thanh WalkSpeed
                        -- WalkSpeed 16 ~ Speed 0.8 (Chậm)
                        -- WalkSpeed 100 ~ Speed 5.0 (Siêu nhanh)
...
function RefreshPlayers()
    ...
    -- [ĐÃ XÓA] Slider chỉnh tốc độ bay riêng lẻ, giờ sẽ dùng chung với WalkSpeed
