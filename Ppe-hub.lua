-- [[ PHUC NGUYEN X HUB - FINAL V10.6 - DEFAULT HITBOX 50 ]] --
...
-- [CẬP NHẬT] Chỉnh mặc định Size Hitbox lên 50
getgenv().Config = {
    ...
    while task.wait() do 
        pcall(function()
            if getgenv().Config.Aura then
                for _, v in pairs(Players:GetPlayers()) do
                    if v ~= LocalPlayer and not getgenv().Whitelist[v.Name] and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                        -- [SỬA ĐỔI] Tối ưu Aura quét theo Size Hitbox để ăn dame cực nhạy
                        local Range = (getgenv().Config.Size / 2) + 7
                        if (LocalPlayer.Character.HumanoidRootPart.Position - v.Character.HumanoidRootPart.Position).Magnitude < Range then
...
Notify("PHUC NGUYEN HUB V10.6 - HITBOX DEFAULT 50!")
