-- [[ PHUC NGUYEN X HUB - FINAL V9.0 - SINK & STOMP MEGA FIX ]] --

-- === [ HỆ THỐNG KIỂM TRA PHIÊN BẢN ] ===
local CurrentVersion = 9.0
...
-- [[ LOGIC CHÍNH: AUTO KILL & SINK MEGA FIX ]] --
spawn(function()
    local Angle = 0
    local TempAuraOff = false
    local SafeY = nil -- Biến lưu trữ Y an toàn
    ...
    while task.wait() do
        ...
        if MyChar and MyChar:FindFirstChild("HumanoidRootPart") and MyChar:FindFirstChild("Humanoid") then
            ...
            -- KIỂM TRA TRẠNG THÁI RAGDOLL
            local IsRagdoll = (MyHum:GetState() == Enum.HumanoidStateType.Physics or MyHum.FloorMaterial == nil)
            
            if IsRagdoll and not getgenv().Config.SkyWalk then
                -- LƯU LẠI TỌA ĐỘ Y AN TOÀN TRƯỚC KHI LÚN
                if not SafeY then SafeY = MyRoot.Position.Y end
                
                -- ÉP LÚN XUYÊN ĐẤT (Y -25 stud)
                MyRoot.Velocity = Vector3.zero
                MyRoot.AssemblyLinearVelocity = Vector3.zero
                MyRoot.CFrame = CFrame.new(MyRoot.Position.X, -25, MyRoot.Position.Z)
                KillStatus.Text = ">>> ĐANG LÚN XUYÊN ĐẤT NÉ ĐÒN <<<"
            else
                -- NẾU ĐÃ ĐỨNG DẬY, TP LẠI CHỖ CŨ (SAFE Y)
                if SafeY then
                    MyRoot.CFrame = CFrame.new(MyRoot.Position.X, SafeY, MyRoot.Position.Z)
                    SafeY = nil -- Reset sau khi TP xong
                    StandUp()
                end
                ...
                if getgenv().TargetKill then
                    ...
                    if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        -- LOGIC ĐẠP KẾT LIỄU (MÁU < 3%)
                        if TarHum.Health > 0 and TarHum.Health < 3 then
                            KillStatus.Text = ">>> ĐANG ĐẠP KẾT LIỄU! <<<"
                            -- Ép bản thân đứng thẳng, dán chặt thân địch
                            StandUp()
                            MyRoot.CFrame = TarRoot.CFrame 
                            MyRoot.Velocity = Vector3.zero
                            
                            -- Tạm ngắt Aura để dồn E
                            if getgenv().Config.Aura then TempAuraOff = true; getgenv().Config.Aura = false end
                        ...
                        elseif TarHum.Health > 0 then
                            -- AUTO KILL BÌNH THƯỜNG
                            if TempAuraOff then getgenv().Config.Aura = true; TempAuraOff = false end
                            ...
                            if Dist > 10 then 
                                ...
                                local SpeedCalc = getgenv().Config.WalkSpeed * 0.05 
                                ...
                                MyRoot.CFrame = CFrame.new(TarRoot.Position + Offset, TarRoot.Position)
                            end
                        end
                    else
                        getgenv().TargetKill = nil
...
Notify("PHUC NGUYEN HUB V9.0 - MEGA FIX LOADED!")
