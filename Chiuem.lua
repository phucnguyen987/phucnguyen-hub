...
local CurrentVersion = 11.0 -- Đã cập nhật phiên bản
...
-- Cập nhật Config mới (Không xóa cái cũ)
getgenv().Config = {
...
-- Menu Đặc Biệt Mới (Vietnam Special)
local SpecialMenu = Instance.new("Frame", ScreenGui); SpecialMenu.Size = UDim2.new(0, 260, 0, 320); SpecialMenu.Position = UDim2.new(0.5, 150, 0.5, -160); SpecialMenu.BackgroundColor3 = Color3.fromRGB(25, 20, 20); SpecialMenu.Visible = false; Instance.new("UICorner", SpecialMenu); MakeRainbowStroke(SpecialMenu, 2)
...
RunService.Heartbeat:Connect(function()
    if getgenv().Config.StreamerMode and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head") then
        for _, v in pairs(LocalPlayer.Character.Head:GetChildren()) do
            if v:IsA("BillboardGui") and v.Name ~= "PhucVipTag" then v.Enabled = false end
        end
    end
end)
...
local EditTag = Instance.new("TextBox", SpecContainer); EditTag.Size = UDim2.new(1,0,0,35); EditTag.PlaceholderText = "Nhập chữ VIP (VD: [TRÙM])"; EditTag.Text = ""; EditTag.BackgroundColor3 = Color3.fromRGB(45,45,45); EditTag.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", EditTag)
EditTag.FocusLost:Connect(function() getgenv().Config.VipTag = EditTag.Text end)
local ColorBtn = Instance.new("TextButton", SpecContainer); ColorBtn.Size = UDim2.new(1,0,0,35); ColorBtn.BackgroundColor3 = Color3.new(1,1,0); ColorBtn.Text = "Đổi Màu Tag (Random)"; ColorBtn.TextColor3 = Color3.new(0,0,0); Instance.new("UICorner", ColorBtn)
...
AddSpecToggle("Streamer Mode (Ẩn tên)", "StreamerMode")
local BuyRemoteBtn = Instance.new("TextButton", SpecContainer); BuyRemoteBtn.Size = UDim2.new(1, 0, 0, 35); BuyRemoteBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 0); BuyRemoteBtn.Text = "🛒 MỞ SHOP TỪ XA (Buy Item)"; BuyRemoteBtn.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", BuyRemoteBtn)
