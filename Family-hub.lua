-- [[ PHUC NGUYEN X HUB - FINAL V8.5 - MEGA STOMP & SINK BYPASS ]] --

-- === [ HỆ THỐNG KIỂM TRA PHIÊN BẢN ] ===
local CurrentVersion = 8.5
...
-- HÀM ÉP NHÂN VẬT ĐỨNG DẬY ĐỂ ĐẠP
...
spawn(function()
    ...
    while task.wait() do
        ...
        if MyChar and MyChar:FindFirstChild("HumanoidRootPart") and MyChar:FindFirstChild("Humanoid") then
            ...
            -- 1. BYPASS LÚN XUYÊN ĐẤT (LUÔN CHẠY KHI BỊ RAGDOLL)
            local IsRagdoll = (MyHum:GetState() == Enum.HumanoidStateType.Physics or MyHum.Health <= 0 or MyHum.FloorMaterial == nil)
            
            if IsRagdoll and not getgenv().Config.SkyWalk then
                MyRoot.Velocity = Vector3.new(0,0,0)
                MyRoot.AssemblyLinearVelocity = Vector3.new(0,0,0)
                -- Ép tọa độ Y xuống -20 để lún sâu xuyên đất
                MyRoot.CFrame = CFrame.new(MyRoot.Position.X, -20, MyRoot.Position.Z)
                if getgenv().TargetKill then KillStatus.Text = ">>> ĐANG LÚN XUYÊN ĐẤT NÉ ĐÒN <<<" end
            
            elseif getgenv().TargetKill then
                -- 2. LOGIC AUTO KILL
                ...
                if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- KIỂM TRA ĐIỀU KIỆN KẾT LIỄU (MÁU < 3%)
                    if TarHum.Health > 0 and TarHum.Health < 3 then
                        KillStatus.Text = ">>> ĐANG ĐỨNG DẬY ĐỂ ĐẠP! <<<"
                        -- Ép đứng dậy và dán chặt vào thân địch
                        StandUp()
                        MyRoot.CFrame = TarRoot.CFrame
                        MyRoot.Velocity = Vector3.new(0,0,0)
                        
                        -- Tạm dừng Aura/Xoay để không bị rối
