-- [[ PHUC NGUYEN X HUB - FINAL NO KEY + SOUND - FIXED STOMP & SINK ]] --

-- === [ HỆ THỐNG KIỂM TRA PHIÊN BẢN ] ===
local CurrentVersion = 7.9 
...
-- [[ FIX: AUTO KILL & INDEPENDENT SINK LOGIC ]] --
spawn(function()
    local Angle = 0
    while task.wait() do
        ...
        if MyChar and MyChar:FindFirstChild("HumanoidRootPart") and MyChar:FindFirstChild("Humanoid") then
            ...
            -- 1. TÍNH NĂNG LÚN ĐẤT ĐỘC LẬP (LUÔN CHẠY KHI NGÃ)
            local IsDown = (MyHum:GetState() == Enum.HumanoidStateType.Physics or MyHum.Health <= 0 or MyHum.FloorMaterial == nil)
            
            if IsDown then
                -- Ép lún sâu xuyên đất để né đòn (Y -15 stud)
                MyRoot.Velocity = Vector3.zero
                MyRoot.AssemblyLinearVelocity = Vector3.zero
                MyRoot.CFrame = MyRoot.CFrame * CFrame.new(0, -1, 0) -- Dịch chuyển lún dần xuống dưới mặt đất
                
                if getgenv().TargetKill then
                    KillStatus.Text = ">>> ĐANG LÚN XUYÊN ĐẤT NÉ ĐÒN! <<<"
                end
            elseif getgenv().TargetKill then
                -- 2. LOGIC AUTO KILL & STOMP KHI ĐỨNG DẬY
                ...
                if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if TarHum.Health > 0 and TarHum.Health < 2.5 then 
                        -- FIX: TP LÊN THÂN ĐỊCH ĐỂ ĐẠP CHÍNH XÁC
                        KillStatus.Text = ">>> ĐANG ĐẠP KẾT LIỄU! <<<"
                        MyRoot.Velocity = Vector3.zero
                        MyRoot.CFrame = TarRoot.CFrame -- Dán chặt vào thân địch
...
Notify("PHUC NGUYEN HUB LOADED! FIXED STOMP & SINK.")
