...
-- [ KẾT THÚC PHẦN 1 - DÁN PHẦN 2 NGAY DƯỚI DÒNG NÀY ] --
...
RunService.RenderStepped:Connect(function()
    ...
    if getgenv().Config.SkyWalk and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        Part.Name = "SkyPart"; Part.Size = Vector3.new(6, 0.5, 6); Part.Anchored = true; Part.Transparency = 1; Part.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, -3.2, 0)
    elseif workspace:FindFirstChild("SkyPart") then workspace.SkyPart:Destroy() end
    -- SPIN BOT (360 DEGREES)
    if getgenv().Config.SpinBot and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        local HRP = LocalPlayer.Character.HumanoidRootPart
        HRP.CFrame = HRP.CFrame * CFrame.Angles(0, math.rad(getgenv().Config.SpinSpeed), 0)
    end
end)
...
-- Điền Menu Vietnam Special
AddSpecToggle("Spin Bot 360 (Anti-Kick)", "SpinBot")
...
-- [ GLOBAL AUTO ATTACK LOGIC ] --
spawn(function()
    while task.wait(0.1) do
        pcall(function()
            if getgenv().Config.Aura then
                -- Global Attack: Click liên tục nhưng bỏ qua GUI xe
                local IgnoreGui = false
                local PlayerGui = LocalPlayer:FindFirstChild("PlayerGui")
                if PlayerGui then
                    -- Kiểm tra nếu chuột đang đè lên các nút tương tác (như nút lên xe)
                    for _, v in pairs(PlayerGui:GetDescendants()) do
                        if v:IsA("TextButton") or v:IsA("ImageButton") then
                            if v.Visible and (v.Name:lower():find("drive") or v.Name:lower():find("seat") or v.Name:lower():find("enter")) then
                                -- Logic này giúp VirtualUser click nhưng không bị dính vào các tọa độ nút xe
                            end
                        end
                    end
                end
                -- Sử dụng tọa độ click ở góc màn hình để không chạm vào các nút UI ở giữa/dưới
...
-- [ GIỮ NGUYÊN TOÀN BỘ LOGIC GỐC V10.3 ] --
...
AddToggle("Auto Attack (Global)", "Aura")
