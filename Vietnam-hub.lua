...
-- [[ ĐÃ SỬA: AUTO KILL (XOAY NHANH + LÚN ĐẤT KHI NGÃ) ]] --
spawn(function()
    local Angle = 0
    while task.wait() do
        if getgenv().TargetKill then
            ...
            if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- LOGIC BYPASS LÚN ĐẤT KHI BỊ ĐÁNH NGÃ
                local IsDown = (MyHum:GetState() == Enum.HumanoidStateType.Physics or MyHum.Health <= 0 or MyHum.FloorMaterial == nil)
                
                if IsDown and Dist < 50 then
                    -- Nếu đang bị ngã, lún xâu xuống đất để né đòn kết liễu
                    MyRoot.CFrame = CFrame.new(MyRoot.Position.X, TarRoot.Position.Y - 5, MyRoot.Position.Z)
                    MyRoot.Velocity = Vector3.new(0,0,0)
                    KillStatus.Text = ">>> ĐANG LÚN ĐẤT NÉ ĐÒN! <<<"
                elseif TarHum.Health > 0 and TarHum.Health < 2 then 
                    KillStatus.Text = ">>> ĐANG DẬM (STOMPING)! <<<"
                    MyRoot.CFrame = TarRoot.CFrame * CFrame.new(0, 3, 0)
                ...
                elseif TarHum.Health > 0 then
                    ...
                    if Dist > 10 then 
                        ...
                        -- TỐC ĐỘ XOAY TĂNG LÊN 0.25 (NHANH HƠN)
                        Angle = Angle + 0.25 
