...
local function AddSpinToggle(Text, Flag)
    local Btn = Instance.new("TextButton", SpinContainer); Btn.Size = UDim2.new(1, 0, 0, 35); Btn.BackgroundColor3 = Color3.fromRGB(40, 40, 40); Btn.Text = Text .. " [OFF]"; Btn.TextColor3 = Color3.new(1,1,1); Btn.Font = Enum.Font.SourceSansBold; Instance.new("UICorner", Btn)
    Btn.MouseButton1Click:Connect(function() PlayClickSound(); getgenv().Config[Flag] = not getgenv().Config[Flag]; Btn.Text = Text .. " [" .. (getgenv().Config[Flag] and "ON" or "OFF") .. "]"; Btn.TextColor3 = getgenv().Config[Flag] and Color3.new(0,1,0) or Color3.new(1,1,1) end)
end
local function AddSpinSlider(Text, Flag, Min, Max)
    ...
    B.MouseButton1Down:Connect(function() local M = LocalPlayer:GetMouse(); local C; C = RunService.RenderStepped:Connect(function() if not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then C:Disconnect() return end local P = math.clamp((M.X - B.AbsolutePosition.X) / B.AbsoluteSize.X, 0, 1); local V = math.floor(Min + (Max - Min) * P); getgenv().Config[Flag] = V; L.Text = Text .. ": " .. V; Fill.Size = UDim2.new(P, 0, 1, 0) end) end)
end
AddSpinToggle("Bật Xoay 360°", "Spin"); AddSpinSlider("Tốc Độ Xoay", "SpinSpeed", 0, 100)
...
BuyX30Btn.MouseButton1Click:Connect(function() PlayClickSound(); local args = {"b\196\131ng g\225\186\161c", 30} pcall(function() game:GetService("ReplicatedStorage"):WaitForChild("KnitPackages"):WaitForChild("_Index"):WaitForChild("sleitnick_knit@1.7.0"):WaitForChild("knit"):WaitForChild("Services"):WaitForChild("ShopService"):WaitForChild("RE"):WaitForChild("buyItem"):FireServer(unpack(args)) end) Notify("Đã thực thi mua X30!") end)
...
function AddToggle(Text, Flag)
    local Btn = Instance.new("TextButton", Container); Btn.Size = UDim2.new(1, 0, 0, 35); Btn.BackgroundColor3 = Color3.fromRGB(30, 30, 30); Btn.Text = "  " .. Text .. " [OFF]"; Btn.TextColor3 = Color3.fromRGB(150, 150, 150); Btn.TextXAlignment = "Left"; Btn.Font = Enum.Font.SourceSansBold
    Btn.MouseButton1Click:Connect(function() PlayClickSound(); getgenv().Config[Flag] = not getgenv().Config[Flag]; Btn.Text = "  " .. Text .. " [" .. (getgenv().Config[Flag] and "ON" or "OFF") .. "]"; Btn.TextColor3 = getgenv().Config[Flag] and Color3.new(0,1,0) or Color3.fromRGB(150,150,150); Notify((getgenv().Config[Flag] and "✅ ĐÃ BẬT: " or "❌ ĐÃ TẮT: ") .. Text) end)
end
function AddSlider(Text, Flag, Min, Max)
    local F = Instance.new("Frame", Container); F.Size = UDim2.new(1, 0, 0, 45); F.BackgroundTransparency = 1; local L = Instance.new("TextLabel", F); L.Size = UDim2.new(1, 0, 0, 20); L.Text = Text .. ": " .. getgenv().Config[Flag]; L.TextColor3 = Color3.new(1,1,1); L.BackgroundTransparency = 1; local B = Instance.new("TextButton", F); B.Size = UDim2.new(0.9, 0, 0, 8); B.Position = UDim2.new(0.05, 0, 0.6, 0); B.BackgroundColor3 = Color3.new(0,0,0); B.Text = ""; local Fill = Instance.new("Frame", B); Fill.Size = UDim2.new(getgenv().Config[Flag]/Max,0,1,0); Fill.BackgroundColor3 = Color3.new(1,0,0)
    B.MouseButton1Down:Connect(function() local M = LocalPlayer:GetMouse(); local C; C = RunService.RenderStepped:Connect(function() if not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then C:Disconnect() return end local P = math.clamp((M.X - B.AbsolutePosition.X) / B.AbsoluteSize.X, 0, 1); local V = math.floor(Min + (Max - Min) * P); getgenv().Config[Flag] = V; L.Text = Text .. ": " .. V; Fill.Size = UDim2.new(P, 0, 1, 0) end) end)
end

AddToggle("Hitbox (No Fling)", "Hitbox"); AddSlider("Kích Thước Hitbox", "Size", 2, 100); AddToggle("Attack Aura", "Aura"); AddToggle("Speed Bypass 500", "Speed"); AddSlider("Tốc Độ", "WalkSpeed", 16, 500); AddToggle("Sky Walk", "SkyWalk"); AddToggle("Anti-Void", "AntiVoid"); AddToggle("Anti-Ragdoll", "NoRagdoll")
    ...
    pcall(function()
        ...
        if HRP and Hum then
            if getgenv().Config.Speed and Hum.MoveDirection.Magnitude > 0 then
                local Extra = (getgenv().Config.WalkSpeed / 16) * 0.55
                HRP.CFrame = HRP.CFrame + (Hum.MoveDirection * Extra)
            end
            if getgenv().Config.Spin then
                SpinAngle = SpinAngle + (getgenv().Config.SpinSpeed / 5)
                HRP.CFrame = HRP.CFrame * CFrame.Angles(0, math.rad(SpinAngle), 0)
            end
            if getgenv().Config.SkyWalk then
                if UserInputService:IsKeyDown(Enum.KeyCode.Space) then HRP.Velocity = Vector3.new(HRP.Velocity.X, 45, HRP.Velocity.Z) else HRP.Velocity = Vector3.new(HRP.Velocity.X, -1.5, HRP.Velocity.Z) end
            end
...
Toggle.MouseButton1Click:Connect(function() PlayClickSound(); Main.Visible = not Main.Visible; SubMenu.Visible = false; SubMenu2.Visible = false; SubMenu3.Visible = false end)
HideBtn.MouseButton1Click:Connect(function() PlayClickSound(); GUIVisible = not GUIVisible; Toggle.Visible = GUIVisible; Main.Visible = false; SubMenu3.Visible = false; HideIcon.Text = GUIVisible and "👁️" or "🚫" end)
Notify("PHUC NGUYEN V10.3 LOADED!")
