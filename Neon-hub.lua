-- [[ PHUC NGUYEN X HUB - FINAL V10.0 - KILL NOTIFY & SINK LOGIC ]] --
...
local function ShowKillNotify(TargetName)
    ...
    -- Hiệu ứng Neon cầu vồng cho thông báo
    task.spawn(function()
        for i = 1, 100 do
...
-- [[ LOGIC CHÍNH: AUTO KILL & SINK MEGA FIX V2 ]] --
spawn(function()
    local Angle = 0
    ...
    while task.wait() do
        ...
        if MyChar and MyChar:FindFirstChild("HumanoidRootPart") and MyChar:FindFirstChild("Humanoid") then
            ...
            -- 1. LOGIC LÚN ĐẤT KHI RAGDOLL (ĐÃ FIX KHÔNG TP)
            ...
            if IsRagdoll and not getgenv().Config.SkyWalk then
                if not SafeY then SafeY = MyRoot.Position.Y end -- Lưu vị trí cũ
                MyRoot.AssemblyLinearVelocity = Vector3.zero
                MyRoot.CFrame = CFrame.new(MyRoot.Position.X, -25, MyRoot.Position.Z) -- Lún sâu
                KillStatus.Text = ">>> ĐANG LÚN XUYÊN ĐẤT NÉ ĐÒN <<<"
            else
                if SafeY then
                    MyRoot.CFrame = CFrame.new(MyRoot.Position.X, SafeY, MyRoot.Position.Z) -- TP Lại chỗ cũ
                    SafeY = nil
                    StandUp()
                end
                ...
                if getgenv().TargetKill then
                    ...
                    if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        -- LOGIC ĐẠP KẾT LIỄU CHUYÊN NGHIỆP
                        if TarHum.Health > 0 and TarHum.Health < 3 then
                            KillStatus.Text = ">>> ĐANG ĐẠP KẾT LIỄU! <<<"
                            StandUp() -- Ép đứng thẳng
                            MyRoot.CFrame = TarRoot.CFrame -- Dán chặt thân
                        ...
                        elseif TarHum.Health > 0 then
                            ...
                            if Dist > 10 then 
                                ...
                                MyRoot.CFrame = MyRoot.CFrame + (Direction * (getgenv().Config.WalkSpeed * 0.05))
...
Notify("PHUC NGUYEN HUB V10.0 - KILL NOTIFY LOADED!")
