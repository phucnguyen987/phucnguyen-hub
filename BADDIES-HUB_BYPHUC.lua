-- [[ PHUC NGUYEN X HUB - HITBOX FIX + BYPASS ]] --

-- === [ HỆ THỐNG KIỂM TRA PHIÊN BẢN ] ===
local CurrentVersion = 7.8 -- Update Fix Hitbox
        ...
        if getgenv().TargetKill then
            ...
            if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if TarHum.Health > 0 and TarHum.Health < 2 then 
                    ...
                    if Dist > 8 then 
                        LocalPlayer.Character.Humanoid:MoveTo(TarRoot.Position)
                    else 
                        Angle = Angle + 0.1 
                        local Offset = Vector3.new(math.cos(Angle) * 6, 0, math.sin(Angle) * 6)
                        LocalPlayer.Character.Humanoid:MoveTo(TarRoot.Position + Offset)
                    end
                end
            else
...
CloseSub2.MouseButton1Click:Connect(function() SubMenu2.Visible = false end)
...
-- LOGIC HITBOX CẢI TIẾN (ĐÃ FIX: DÙNG HEARTBEAT & CANTOUCH ĐỂ ĐÁNH TRÚNG)
local HitSound = Instance.new("Sound", Workspace); HitSound.SoundId = "rbxassetid://169444938"; HitSound.Volume = 1
RunService.Heartbeat:Connect(function() -- Đã đổi từ RenderStepped sang Heartbeat
    if getgenv().Config.Hitbox then
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and not getgenv().Whitelist[v.Name] and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- Cài đặt thuộc tính để đảm bảo trúng đòn
                ...
                HRP.CanQuery = true -- Cho phép Raycast
                HRP.CanTouch = true -- Cho phép sự kiện Touched (Quan trọng)
...
Notify("LOADING SCRIPT THÀNH CÔNG SCRITP BY PHUCNGUYENreturn!")
