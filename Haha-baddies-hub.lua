-- [[ PHUC NGUYEN X HUB - FINAL V10.4 - FIX HITBOX & NAMES ]] --
                ...
                if getgenv().TargetKill then
                    ...
                    if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if TarHum.Health > 0 and TarHum.Health < 3 then
                            KillStatus.Text = ">>> ĐANG ĐỨNG ĐẬP KẾT LIỄU! <<<"
                            StandUp()
                            MyRoot.CFrame = TarRoot.CFrame -- Dán chặt thân địch
                        ...
                        elseif TarHum.Health > 0 then
                            -- CHẾ ĐỘ CHIẾN ĐẤU XOAY VÒNG
                            if AuraBackup then getgenv().Config.Aura = true; AuraBackup = false end
                            KillStatus.Text = ">>> ĐANG SĂN: " .. Target.DisplayName .. " <<<"
                            ...
                            if Dist > 10 then 
                                ...
                                -- [SỬA ĐỔI] Tăng tốc độ xoay (0.3 -> 1.5)
                                Angle = Angle + 1.5 
...
function RefreshPlayers()
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer then
            ...
            -- [SỬA ĐỔI] HIỂN THỊ DISPLAY NAME THAY VÌ USERNAME
            local PName = Instance.new("TextLabel", PFrame); PName.Size = UDim2.new(0.35, 0, 1, 0); PName.BackgroundTransparency = 1; 
            PName.Text = v.DisplayName; -- Sử dụng Tên Hiển Thị (Display Name)
            PName.TextColor3 = Color3.new(1,1,1); PName.Font = Enum.Font.SourceSansBold; PName.TextSize = 13; PName.TextXAlignment = Enum.TextXAlignment.Left; PName.Position = UDim2.new(0.02,0,0,0)
            ...
            -- Logic bên dưới vẫn dùng v.Name để định danh chính xác người chơi
...
spawn(function()
    -- [SỬA ĐỔI] Tăng tốc độ vòng lặp Aura để đánh trúng Hitbox to
    while task.wait() do -- Xóa số 0.1 để chạy max tốc độ (Fastest)
        pcall(function()
            if getgenv().Config.Aura then
                for _, v in pairs(Players:GetPlayers()) do
                    if v ~= LocalPlayer and not getgenv().Whitelist[v.Name] and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                        -- Check khoảng cách với Hitbox to
                        if (LocalPlayer.Character.HumanoidRootPart.Position - v.Character.HumanoidRootPart.Position).Magnitude < (getgenv().Config.Size + 5) then
                            ...
                            VirtualUser:Button1Down(Vector2.new(9999, 9999)) -- Dùng Down/Up để chắc ăn hơn Click
...
Notify("PHUC NGUYEN HUB V10.4 - FIX HITBOX & NAME!")
